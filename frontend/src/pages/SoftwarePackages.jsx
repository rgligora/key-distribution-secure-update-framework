...
import { fetchDataWithRequestParams, createData, deleteData } from '../api.js';
...
const SoftwarePackages = ({ companyId }) => {
  ...
  const handleDeleteSoftwarePackage = async (softwarePackageId) => {
    try {
      const successMessage = await deleteData(`software-packages/${softwarePackageId}`);
      const updatedSoftwarePackages = softwarePackagesData.filter(sp => sp.softwarePackageId !== softwarePackageId);
      setsoftwarePackagesData(updatedSoftwarePackages);
      setIsDetailModalOpen(false); 
      setDeleteError('');
    } catch (error) {
      console.error('Failed to delete software package:', error);
  ...
  return (
    <div className='m-2 md:m-10 p-2 md:p-10 bg-white rounded-3xl'>
      ...
      <Modal isOpen={isDetailModalOpen} onClose={() => handleCloseModal()} title={`Details for: ${selectedSoftwarePackage?.name}`}>
        <div>
          ...
          <button className="mt-5 mb-5 p-3 bg-red-600 text-white rounded-lg hover:bg-red-800" onClick={() => handleDeleteSoftwarePackage(selectedSoftwarePackage.softwarePackageId)}>
